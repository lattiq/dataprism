...
class DataPrism:
    ...
    def analyze(
        ...
        if column_configs and not requested_columns and schema is not None:
            ...
            logger.info(
                "Using explicitly provided columns list with %s features", len(requested_columns)
            )
        ...
        if exclude_columns:
            ...
            if protected & set(exclude_columns):
                logger.info(
                    "Kept protected columns from exclude list: %s",
                    sorted(str(c) for c in protected & set(exclude_columns)),
                )
        ...
        if requested_columns:
            ...
            if column_configs:
                ...
                logger.info(
                    "Schema defined for %s columns, %s columns will use auto-detected types",
                    with_schema,
                    without_schema,
                )
            ...
            if (
                target_variable
                and target_variable in df.columns
                and target_variable not in available_columns
            ):
            ...
            if (
                ...
                logger.info(
                    "Keeping cohort column for stability calculation: %s", self.cohort_column
                )
        ...
        logger.info(
            "Analyzing %s features...",
            len(available_columns) if available_columns else len(df.columns),
        )
        ...
        if self.calculate_stability and self.cohort_column:
            if self.cohort_column in df.columns:
                try:
                    ...
                    for name, feature_result in features.items():
                        ...
                        if ftype:
                            ...
                            ftype = (
                                "continuous" if inferred is ColumnType.CONTINUOUS else "categorical"
                            )
                    ...
                    features_to_analyze = [f for f in features if f != target_variable]
                ...
                logger.warning(
                    " Cohort column '%s' not found. Skipping cohort-based stability calculation.",
                    self.cohort_column,
                )
        ...
        if self.time_based_stability and self.time_column:
            if self.time_column in df.columns:
                try:
                    ...
                    features_to_analyze = [
                        f for f in features if f != self.time_column and f != target_variable
                    ]
                ...
                logger.warning(
                    " Time column '%s' not found. Skipping time-based stability calculation.",
                    self.time_column,
                )
        ...
        if provider_match_rates:
            ...
            logger.info(
                "  No provider match rates computed (no metadata or record_not_found columns found)"
            )
